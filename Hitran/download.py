...
class WebApi(object):
    ...
    def download_transitions(self, isotopologues, numin, numax, parameters=None):
        ...
        for line in data.split("\n"):
            ...
            transitions.append("\t".join(line.rstrip().split(",")))
        return transitions
...
def run(fn, api_key=None, molecules=None, fmin=0., fmax=3e20):
    ...
    ----------
    fn : PathLike
        Path for output hitran file.
    api_key : str
        Your personal API key from the Hitran webpage
    molecules : list[str], NoneType
        Molecules to download (checks for ordinary formula, e.g., H2O).  None indicates all...
    fmin : float
        Numeric minimum frequency (Hz).  Min is 0. Max is 3e20 Hz.
    fmax : float
    ...
    a.close()
    hapi = WebApi(api_key)
    for molecule in hapi.download_molecules():
        if molecules is not None and molecule.ordinary_formula not in molecules:
            continue
        
        a = open(fn, 'a')
        print(f"Trying to download {molecule.ordinary_formula} lines between {numin} and {numax} cm-1")
        try:
            iso = hapi.download_isotopologues(molecule)
            t = hapi.download_transitions(iso, numin=numin, numax=numax,
                                          parameters=["par_line", "statep", "statepp"])
            print(f"Got {len(t)} lines for {molecule.ordinary_formula}")
            for x in t:
                a.write(x)
                a.write('\n')
        except:
            print(f"Got no lines for {molecule.ordinary_formula}")
        a.close()